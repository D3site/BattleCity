...
std::shared_ptr<RenderEngine::Texture2D> ResourceManager::loadTexture(const std::string& textureName, const std::string& texturePath)
{
	...
	int width{ 0 };
	int height{ 0 };
	stbi_set_flip_vertically_on_load(true);//из-за того, что опенгл считывается снизу слева, а нам нужно сверху, поэтому мы переворачиваем картинку
	...
	const std::string subTextureName)
{
	...
	if (!pShader)
	{
		std::cerr << "Can't find the texture: " << shaderName <<"for the sprite " << pShader << std::endl;
	}
	...
	if (animatedSpritesIt != document.MemberEnd())
	{
		for (const auto& currentAnimatedSprite : animatedSpritesIt->value.GetArray())
		{
			...
			for (const auto& currentState : statesArray)
			{
				...
				pAnimatedSprite->intertState(stateName, std::move(frames));
			}
		}
	}
	return true;
